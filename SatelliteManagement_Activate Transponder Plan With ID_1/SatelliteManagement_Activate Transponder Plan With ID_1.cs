...
namespace Activate_Transponder_Plan_With_ID_1
{
	using System;
	...
	public class Script
	{
		private const string ScriptName = "SAT-AS-Activate Transponder Plan With ID";
		...
		public void Run(IEngine engine)
		{
			using (var logger = new SatOpsLogger(SatOpsLogger.Types.Satellite))
			{
				try
				{
					...
					var domTransponderPlanId = engine.ReadScriptParamSingleFromApp<Guid>("Transponder Plan Id");
					...
					if (transponderPlan.DomTransponderPlan.StatusId.Equals("edit") || transponderPlan.DomTransponderPlan.StatusId.Equals("draft") || transponderPlan.DomTransponderPlan.StatusId.Equals("deprecated"))
					{
						transponderPlan.Activate();
					}
				}
				catch (Exception ex)
				{
					logger.Error(ex, $"Exception occurred in '{ScriptName}'");
					engine.ShowErrorDialog($"An error occurred while trying to activate transponder plan: {ex.Message}");
				}
			}
		}
	}
