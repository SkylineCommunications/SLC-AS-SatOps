namespace Skyline.DataMiner.Utils.SatOps.Common.IOData.SatelliteManagement.Scripts.BeamHandler
{
	...
	public class InputData
	{
		...
		/// <summary>
		/// Sends the input data to the beam handler.
		/// </summary>
		...
		public OutputData SendToBeamHandler(IEngine engine, bool useSerialization = false)
		{
			var data = new BeamHandlerData
			{
				Input = this,
			};
			...
			if (useSerialization)
			{
				data.PrepareJsonCommunication(new KnownTypesBinder(), "BeamHandler Result");
			}

			var subScript = engine.PrepareSubScript("SatelliteManagement_Core_BeamHandler");
			...
			if (data.Communication == DataMiner.MediaOps.Communication.ScriptData.ScriptDataBase.CommunicationType.Json)
			{
				...
				data = JsonConvert.DeserializeObject<BeamHandlerData>(jsonData, new JsonSerializerSettings { TypeNameHandling = TypeNameHandling.Objects, SerializationBinder = new KnownTypesBinder() });
			}
