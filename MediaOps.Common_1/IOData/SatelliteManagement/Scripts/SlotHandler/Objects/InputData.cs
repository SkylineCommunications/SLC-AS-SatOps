namespace Skyline.DataMiner.Utils.SatOps.Common.IOData.SatelliteManagement.Scripts.SlotHandler
{
	...
	public class InputData
	{
		...
		/// <summary>
		/// Sends the input data to the slot handler.
		/// </summary>
		...
		public OutputData SendToSlotHandler(IEngine engine, bool useSerialization = false)
		{
			var data = new SlotHandlerData
			{
				Input = this,
			};
			...
			if (useSerialization)
			{
				data.PrepareJsonCommunication(new KnownTypesBinder(), "SlotHandler Result");
			}

			var subScript = engine.PrepareSubScript("SatelliteManagement_Core_SlotHandler");
			...
			if (data.Communication == DataMiner.MediaOps.Communication.ScriptData.ScriptDataBase.CommunicationType.Json)
			{
				...
				data = JsonConvert.DeserializeObject<SlotHandlerData>(jsonData, new JsonSerializerSettings { TypeNameHandling = TypeNameHandling.Objects, SerializationBinder = new KnownTypesBinder() });
			}
