namespace Skyline.DataMiner.Utils.SatOps.Common.IOData.SatelliteManagement.Scripts.TransponderPlanHandler
{
	...
	public class InputData
	{
		...
		/// <summary>
		/// Sends the input data to the transponder plan handler.
		/// </summary>
		...
		public OutputData SendToTransponderPlanHandler(IEngine engine, bool useSerialization = false)
		{
			var data = new TransponderPlanHandlerData
			{
				Input = this,
			};
			...
			if (useSerialization)
			{
				data.PrepareJsonCommunication(new KnownTypesBinder(), "TransponderPlanHandler Result");
			}

			var subScript = engine.PrepareSubScript("SatelliteManagement_Core_TransponderPlanHandler");
			...
			if (data.Communication == DataMiner.MediaOps.Communication.ScriptData.ScriptDataBase.CommunicationType.Json)
			{
				...
				data = JsonConvert.DeserializeObject<TransponderPlanHandlerData>(jsonData, new JsonSerializerSettings { TypeNameHandling = TypeNameHandling.Objects, SerializationBinder = new KnownTypesBinder() });
			}
